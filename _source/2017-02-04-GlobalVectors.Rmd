...
```{r make_vocabulary, eval = TRUE, echo = TRUE, message = FALSE}
...
```{r denoise_vocabulary, eval = TRUE, message = FALSE}
...
```{r term_cooccurrence_matrix, eval = FALSE, echo = FALSE}
...
```{r tcm_type_conversion, eval = FALSE, echo = FALSE}
...
```{r fitting_glove, eval = FALSE, echo = FALSE}
library(text2vec)
GV <- GloVe$new(
...
```{r getting_glove_vectors, eval = FALSE}
glove_word_vectors <- GV$get_word_vectors()
```

```{r save_vectors, eval = FALSE, echo = FALSE}
save(glove_word_vectors, file = "~/Lab/rsync/plprbt.data/data/plprbt_glove_word_vectors.RData")
```
...
```{r read_vectors, eval = TRUE, echo = FALSE}
load(file = "~/Lab/rsync/plprbt.data/data/plprbt_glove_word_vectors.RData")
```
